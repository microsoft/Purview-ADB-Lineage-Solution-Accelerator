...
val exampleA = (
  ...
  .option("header", true)
  .load(abfssRootPath+"/testcase/fourteen/exampleInputA/exampleInputA.csv")
)
...
val exampleB = (
  ...
  .option("header", true)
  .load(abfssRootPath+"/testcase/fourteen/exampleInputB/exampleInputB.csv")
)
...
outputDf.repartition(1).write.mode("overwrite").format("csv").save(outputRootPath+"/testcase/fourteen/call-via-adf-spark2-folder/")
