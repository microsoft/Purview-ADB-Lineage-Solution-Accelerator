...
val exampleA = (
  ...
  .load(abfssRootPath+"/testcase/four/exampleInputA")
)

val exampleB = (
  spark.read.format("delta")
  .load(abfssRootPath+"/testcase/four/exampleInputB")
)
...
//DOESN'T WORK
outputDf.write.format("delta").mode("append").save(outputRootPath+"/testcase/four/exampleOutput/")