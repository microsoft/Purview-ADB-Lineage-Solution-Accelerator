...
val exampleA = (
  ...
  .option("header", true)
  .load(abfssRootPath+"/testcase/thirteen/exampleInputA/exampleInputA.csv")
)
...
val exampleB = (
  ...
  .option("header", true)
  .load(abfssRootPath+"/testcase/thirteen/exampleInputB/exampleInputB.csv")
)
...
outputDf.repartition(1).write.mode("overwrite").format("csv").save(outputRootPath+"/testcase/thirteen/call-via-adf-spark2-folder/")
