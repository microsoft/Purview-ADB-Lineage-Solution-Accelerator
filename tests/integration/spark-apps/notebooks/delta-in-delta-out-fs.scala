...
val exampleA = (
  ...
  .load(abfssRootPath+"/testcase/five/exampleInputA")
)

val exampleB = (
  spark.read.format("delta")
  .load(abfssRootPath+"/testcase/five/exampleInputB")
)
...
//WORKS
outputDf.write.format("delta").mode("append").save(outputRootPath + "/testcase/five/delta-in-delta-out-fs-folder")